...
bool ResearchProjcectReplaceMagiTest () {
    ...
    ASSERT_EQUALS(0,
                  numberOfCreatures); // magi 3 had 0 creatures. Magi 1 saw that magi 3 was sad, but he had no creatures to give him, so he disapparated, feeling ashamed
    free(creatures);

    ASSERT_EQUALS(SUCCESS, GetAllCreaturesByLevel(r, 33, &creatures, &numberOfCreatures));
    ASSERT_EQUALS(VERY_MEDIUM_NUMBER / 2 - 1,
                  numberOfCreatures); // magi 33 had 0 creatures. Magi 2 saw that magi 33 was sad, so he decided to give him his creatures, and disapparated (very odd..)
    free(creatures);

    ASSERT_EQUALS(SUCCESS, GetAllCreaturesByLevel(r, 4, &creatures, &numberOfCreatures));
    ASSERT_EQUALS(VERY_MEDIUM_NUMBER / 2,
                  numberOfCreatures); // magi 4 had 50 creatures. Magi 11 saw that magi 4 was proud, so he disapparated, feeling not helpful
    free(creatures);

    ASSERT_EQUALS(SUCCESS, GetAllCreaturesByLevel(r, 44, &creatures, &numberOfCreatures));
    ASSERT_EQUALS(VERY_MEDIUM_NUMBER - 1,
                  numberOfCreatures); // magi 44 had 49 creatures. Magi 22 saw that magi 44 was a brother, so he gave him his creatures, and disapparated (thx bro)
    free(creatures);
    ...
    ASSERT_EQUALS(FAILURE,
                  AddCreature(r, DONT_CARE, 1, DONT_CARE)); // magi 1 disappeared, so no creatures for him anymore
...
bool ResearchProjcectGetAllCreaturesByLevelTest () {
    ...
    ASSERT_EQUALS(SUCCESS,
                  AddCreature(r, 1100, 1, 101)); // this creature has the same level as the creature with id 100
    ASSERT_EQUALS(SUCCESS, GetAllCreaturesByLevel(r, 1, &creaturesLevel, &numberOfCreatures));
